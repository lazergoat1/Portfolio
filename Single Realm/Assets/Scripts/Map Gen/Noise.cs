...
public class Noise : MonoBehaviour
{
    ...
    public static float[,] GenerateTileMapNoise(int width, int height, int seed, float offsetX, float offsetY, float scale, int octaves, float persistance, float lacunarity)
    {
        float[,] noiseMap = new float[width, height];
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                for (int i = 0; i < octaves; i++)
                {
                    float xCoord = (float)x / scale * frequency + (octaveOffsets[i].x / scale * frequency);
                    float yCoord = (float)y / scale * frequency + (octaveOffsets[i].y / scale * frequency);
